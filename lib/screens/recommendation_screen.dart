...
const List<MoodCategory> kMoodCategories = [
  MoodCategory(
    ...
    description: 'Overwhelmed, tense, or under pressure',
  ),
  MoodCategory(
    value: 'anxious',
    label: 'Anxious',
    ...
    description: 'Worried, nervous, or restless',
  ),
  MoodCategory(
    value: 'sad',
    label: 'Sad',
    ...
    description: 'Down, low energy, or unmotivated',
  ),
  MoodCategory(
    value: 'lonely',
    label: 'Lonely',
    ...
    description: 'Isolated, disconnected, or alone',
  ),
  MoodCategory(
    value: 'neutral',
    label: 'Neutral',
  ...
  MoodCategory(
    ...
    description: 'Peaceful, relaxed, or centered',
  ),
  MoodCategory(
    value: 'happy',
    label: 'Happy',
    ...
    description: 'Joyful, content, or cheerful',
  ),
  MoodCategory(
    value: 'energized',
    label: 'Energized',
    emoji: '⚡',
    color: Color(0xFF63EB68),
    icon: Icons.bolt_rounded,
    description: 'Motivated, focused, or pumped',
  ),
];
...
class _RecommendationsContentState extends State<RecommendationsContent> {
  ...
  void initState() {
    ...
    // Pre-select current mood if available
    if (widget.mood != null) {
  ...
  Future<void> _loadTestimonials() async {
    ...
    final testimonials = await RatingService.getTestimonials(
      limit: 20,
      mood: _testimonialMoodFilter,
    );
    if (mounted) {
      setState(() {
        _dbTestimonials = testimonials;
  ...
  bool _validateForm() {
    if (_whatWorkedController.text.trim().isEmpty) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
          content: Text("Please tell us what worked for you"),
    ...
    if (_selectedMood == null) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
          content: Text("Please select the mood you were in"),
          behavior: SnackBarBehavior.floating,
          shape: StadiumBorder(),
          backgroundColor: Color(0xFFFF6B6B),
        ),
    ...
    if (_selectedSessionType == null) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
          content: Text("Please select the session type"),
          behavior: SnackBarBehavior.floating,
          shape: StadiumBorder(),
          backgroundColor: Color(0xFFFF6B6B),
        ),
    ...
    if (!_consentRead) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
          content: Text("Please read and acknowledge the consent notice"),
          behavior: SnackBarBehavior.floating,
          shape: StadiumBorder(),
          backgroundColor: Color(0xFFFF6B6B),
        ),
  ...
  Future<void> _submitTestimonial() async {
    ...
    final success = await RatingService.submitTestimonial(
      userId: widget.userId ?? 'anonymous',
      userName: displayName,
      whatWorked: _whatWorkedController.text.trim(),
      description: _descriptionController.text.trim(),
      starRating: _testimonialRating,
      sessionType: _selectedSessionType!,
      moodWhenItWorked: _selectedMood!,
      consentForPublic: _consentForPublic,
      displayNameType: _displayNameType,
    );

    if (success) {
      setState(() => _testimonialSubmitted = true);
      await Future.delayed(const Duration(seconds: 2));
      if (mounted) {
        setState(() {
          _showTestimonialForm = false;
          _testimonialSubmitted = false;
    ...
    if (mounted) {
      setState(() => _isSubmittingTestimonial = false);
    }
  }
      ...
      child: Column(
        ...
        children: [
                          ...
                          Row(
                            children: [
                              Container(
                                ...
                                decoration: BoxDecoration(color: kPrimaryGreen, borderRadius: BorderRadius.circular(8)),
                                child: const Text("Today's Pick", style: TextStyle(fontSize: 11, fontWeight: FontWeight.bold, color: Colors.white)),
                              ),
                              const SizedBox(width: 8),
                              Container(
                                ...
                                child: Row(
                                  mainAxisSize: MainAxisSize.min,
                                  children: [
                                    Icon(Icons.calendar_today_rounded, color: Colors.white70, size: 11),
                                    const SizedBox(width: 4),
                                    Text(_getDayLabel(), style: const TextStyle(fontSize: 11, fontWeight: FontWeight.w600, color: Colors.white70)),
                                  ],
                                ),
                              ),
                            ],
                      ...
                      children: [
                        Container(
                          ...
                          decoration: BoxDecoration(
                            ...
                            shape: BoxShape.circle,
                            boxShadow: [BoxShadow(color: Colors.redAccent.withOpacity(0.4), blurRadius: 12, offset: const Offset(0, 5))],
                          ),
                          child: const Icon(Icons.play_arrow, color: Colors.white, size: 30),
                        ),
                        const SizedBox(height: 6),
                        Text('YouTube', style: TextStyle(fontSize: 10, fontWeight: FontWeight.w700, color: Colors.white.withOpacity(0.6), letterSpacing: 0.5)),
                      ],
                    ),
                  ],
                ),
          ...
          Row(
            ...
            children: [
              ...
              GestureDetector(
                ...
                child: Text(_showAllTips ? 'See Less' : 'See All', style: TextStyle(fontSize: 14, fontWeight: FontWeight.w600, color: kPrimaryGreen)),
              ),
            ],
          ),
          ...
          GridView.count(
            ...
            children: (_showAllTips ? _gridItems : _gridItems.take(4))
                .map((item) => _buildGridItem(item['title']!, item['image']!))
                .toList(),
          ),
            ...
            children: [
              ...
              Row(
                children: [
                  ...
                  const SizedBox(width: 4),
                  Text('YouTube', style: TextStyle(fontSize: 12, fontWeight: FontWeight.w600, color: Colors.grey.shade400)),
                ],
              ),
            ],
          ),
          ...
          ...List.generate(
            _currentMusicList.length,
            (index) => _buildMusicCard(track: _currentMusicList[index]),
          ),
            ...
            child: Column(
              ...
              children: [
                  ...
                  children: [
                    ...
                    const Expanded(
                      child: Column(
                        crossAxisAlignment: CrossAxisAlignment.start,
                        children: [
                          Text('Share Your Experience', style: TextStyle(fontSize: 20, fontWeight: FontWeight.w900, color: Colors.white)),
                          SizedBox(height: 4),
                          Text('Help others discover what worked for you', style: TextStyle(fontSize: 13, fontWeight: FontWeight.w500, color: Colors.white70)),
                        ],
                      ),
                    ),
                  ],
                ...
                Container(
                  ...
                  child: Row(
                    children: [
                      Icon(Icons.shield_moon_rounded, color: Colors.white.withOpacity(0.9), size: 18),
                      const SizedBox(width: 10),
                      Expanded(
                        child: Text(
                          'Your privacy matters. You control how your testimonial is shared.',
          ...
          // ── Testimonial Form ──
          if (_showTestimonialForm)
            _buildTestimonialForm(),

          const SizedBox(height: 35),

          // ── Testimonials Header with Filter ──
          Row(
            mainAxisAlignment: MainAxisAlignment.spaceBetween,
            children: [
              const Text('Community Experiences', style: TextStyle(fontSize: 20, fontWeight: FontWeight.w900, color: Colors.black87)),
              GestureDetector(
                onTap: _loadTestimonials,
                child: Row(
                  children: [
                    Icon(Icons.refresh_rounded, size: 16, color: Colors.grey.shade400),
                    const SizedBox(width: 4),
                    Text('Refresh', style: TextStyle(fontSize: 13, fontWeight: FontWeight.w600, color: Colors.grey.shade400)),
                  ],
                ),
              ),
            ],
          ...
          SizedBox(
            ...
            child: ListView(
              ...
              children: [
                ...
                ...kMoodCategories.map((mood) => _buildMoodFilterChip(
                  mood.value, 
            ...
            Container(
              ...
              child: Column(
                children: [
                  ...
                  const SizedBox(height: 16),
                  Text('No testimonials yet for this mood', style: TextStyle(fontSize: 16, fontWeight: FontWeight.w700, color: Colors.grey.shade400)),
                  const SizedBox(height: 8),
                  Text('Be the first to share your experience!', style: TextStyle(fontSize: 13, fontWeight: FontWeight.w500, color: Colors.grey.shade400)),
                ],
              ),
            )
          else
      ...
      child: GestureDetector(
        ...
        child: AnimatedContainer(
          ...
          child: Row(
            mainAxisSize: MainAxisSize.min,
            children: [
              Icon(icon, size: 14, color: isSelected ? color : Colors.grey.shade400),
              const SizedBox(width: 6),
              Text(label, style: TextStyle(fontSize: 12, fontWeight: FontWeight.w700, color: isSelected ? color : Colors.grey.shade500)),
            ],
          ),
        ),
      ),
  ...
  Widget _buildTestimonialForm() {
    ...
    if (_testimonialSubmitted) {
      return Container(
        ...
        decoration: BoxDecoration(
          ...
          boxShadow: [BoxShadow(color: kPrimaryGreen.withOpacity(0.15), blurRadius: 20)],
        ),
        child: Column(
          children: [
            Container(
              ...
              child: const Icon(Icons.check_circle_rounded, color: Color.fromARGB(255, 99, 235, 104), size: 36),
            ),
            const SizedBox(height: 16),
            const Text('Thank You for Sharing!', style: TextStyle(fontSize: 20, fontWeight: FontWeight.w900, color: Color(0xFF2C3E50))),
            const SizedBox(height: 8),
            const Text('Your testimonial is under review and will appear publicly once approved.', style: TextStyle(fontSize: 13, fontWeight: FontWeight.w500, color: Color(0xFF2C3E50)), textAlign: TextAlign.center),
            const SizedBox(height: 8),
            Container(
              ...
              child: Row(
                mainAxisSize: MainAxisSize.min,
                children: [
                  Icon(Icons.shield, size: 14, color: Colors.grey.shade500),
                  const SizedBox(width: 6),
                  Text('Shared as: ${_displayNameType == "anonymous" ? "Anonymous" : _displayNameType == "first_name" ? "First name only" : "Full name"}', style: TextStyle(fontSize: 11, fontWeight: FontWeight.w600, color: Colors.grey.shade500)),
                ],
              ),
            ),
          ],
    ...
    return Container(
      ...
      decoration: BoxDecoration(
        ...
        border: Border.all(color: kPrimaryGreen.withOpacity(0.3), width: 1.5),
      ),
      child: Column(
        crossAxisAlignment: CrossAxisAlignment.start,
        children: [
          ...
          Row(
            mainAxisAlignment: MainAxisAlignment.spaceBetween,
            children: [
              const Text('Share Your Story', style: TextStyle(fontSize: 20, fontWeight: FontWeight.w900, color: Color(0xFF2C3E50))),
              GestureDetector(
                onTap: () => setState(() => _showTestimonialForm = false),
                child: Container(
          ...
          // ── STEP 1: Mood When It Worked (DROPDOWN) ──
          Container(
            padding: const EdgeInsets.all(12),
            decoration: BoxDecoration(
            ...
            child: Row(
              children: [
                Container(
                  ...
                  child: const Center(child: Text('1', style: TextStyle(fontSize: 12, fontWeight: FontWeight.w900, color: Colors.white))),
                ),
                const SizedBox(width: 10),
                const Text('What mood were you in?', style: TextStyle(fontSize: 14, fontWeight: FontWeight.w700, color: Color(0xFF2C3E50))),
              ],
            ),
          ),
          ...
          Container(
            decoration: BoxDecoration(
              ...
              border: Border.all(
                color: _selectedMood != null 
                    ? kMoodCategories.firstWhere((m) => m.value == _selectedMood).color 
            ...
            child: DropdownButtonFormField<String>(
              ...
              hint: const Row(
                children: [
                  Icon(Icons.mood_rounded, color: Colors.grey, size: 20),
                  SizedBox(width: 10),
                  Text('Select your mood', style: TextStyle(fontSize: 14, fontWeight: FontWeight.w500, color: Colors.grey)),
                ],
              ),
              isExpanded: true,
                  ...
                  child: Row(
                    children: [
                      Container(
                        ...
                        child: Center(child: Text(mood.emoji, style: const TextStyle(fontSize: 18))),
                      ),
                      const SizedBox(width: 12),
                      Expanded(
                        child: Column(
                          ...
                          mainAxisSize: MainAxisSize.min,
                          children: [
                            Text(mood.label, style: const TextStyle(fontSize: 14, fontWeight: FontWeight.w700, color: Color(0xFF2C3E50))),
                            Text(mood.description, style: TextStyle(fontSize: 11, fontWeight: FontWeight.w500, color: Colors.grey.shade500)),
                          ],
                        ),
                      ),
                    ],
          ...
          // ── STEP 2: Session Type (DROPDOWN) ──
          Container(
            padding: const EdgeInsets.all(12),
            decoration: BoxDecoration(
              color: const Color(0xFF5C7CFA).withOpacity(0.08),
              borderRadius: BorderRadius.circular(12),
              border: Border.all(color: const Color(0xFF5C7CFA).withOpacity(0.2)),
            ),
            child: Row(
              children: [
                Container(
                  width: 24, height: 24,
                  decoration: BoxDecoration(color: const Color(0xFF5C7CFA), borderRadius: BorderRadius.circular(8)),
                  child: const Center(child: Text('2', style: TextStyle(fontSize: 12, fontWeight: FontWeight.w900, color: Colors.white))),
                ),
                const SizedBox(width: 10),
                const Text('What type of session helped?', style: TextStyle(fontSize: 14, fontWeight: FontWeight.w700, color: Color(0xFF2C3E50))),
              ],
            ),
          ),
          const SizedBox(height: 12),
          ...
          Container(
            decoration: BoxDecoration(
              ...
              borderRadius: BorderRadius.circular(14),
              border: Border.all(
                color: _selectedSessionType != null 
                    ? (_sessionTypes.firstWhere((t) => t['value'] == _selectedSessionType)['color'] as Color)
                    : Colors.grey.shade200,
                width: 1.5,
              ),
            ),
            child: DropdownButtonFormField<String>(
              ...
              hint: const Row(
                children: [
                  Icon(Icons.self_improvement_rounded, color: Colors.grey, size: 20),
                  SizedBox(width: 10),
                  Text('Select session type', style: TextStyle(fontSize: 14, fontWeight: FontWeight.w500, color: Colors.grey)),
                ],
              ),
              isExpanded: true,
              decoration: const InputDecoration(
                ...
                return DropdownMenuItem<String>(
                  ...
                  child: Row(
                    children: [
                      ...
                      const SizedBox(width: 12),
                      Text(type['value'], style: TextStyle(fontSize: 14, fontWeight: FontWeight.w700, color: Color(0xFF2C3E50))),
                    ],
                  ),
                );
              }).toList(),
          ...
          Container(
            padding: const EdgeInsets.all(12),
            decoration: BoxDecoration(
              color: const Color(0xFF63EB68).withOpacity(0.08),
              borderRadius: BorderRadius.circular(12),
              border: Border.all(color: const Color(0xFF63EB68).withOpacity(0.2)),
            ),
            child: Row(
              children: [
                Container(
                  width: 24, height: 24,
                  decoration: BoxDecoration(color: const Color(0xFF63EB68), borderRadius: BorderRadius.circular(8)),
                  child: const Center(child: Text('3', style: TextStyle(fontSize: 12, fontWeight: FontWeight.w900, color: Colors.white))),
                ),
                const SizedBox(width: 10),
                const Text('What specifically worked? *', style: TextStyle(fontSize: 14, fontWeight: FontWeight.w700, color: Color(0xFF2C3E50))),
              ],
            ),
          ),
          const SizedBox(height: 12),
          Container(
            decoration: BoxDecoration(
              ...
              border: Border.all(color: _whatWorkedController.text.isEmpty ? Colors.grey.shade200 : kPrimaryGreen, width: 1.5),
            ),
            child: TextField(
              ...
              decoration: InputDecoration(
                labelText: 'The key thing that helped you',
                labelStyle: TextStyle(fontWeight: FontWeight.w600, color: Colors.grey.shade500),
                hintText: 'e.g., 4-7-8 breathing before bed, guided body scan meditation',
                hintStyle: TextStyle(fontSize: 13, color: Colors.grey.shade400),
                counterStyle: TextStyle(fontSize: 10, fontWeight: FontWeight.w600, color: Colors.grey.shade400),
                border: InputBorder.none,
                contentPadding: const EdgeInsets.symmetric(horizontal: 16, vertical: 14),
                prefixIcon: Icon(Icons.lightbulb_outline_rounded, color: kPrimaryGreen),
              ),
            ),
          ),
          const SizedBox(height: 16),
          ...
          Container(
            ...
            child: TextField(
              ...
              decoration: InputDecoration(
                ...
                labelStyle: TextStyle(fontWeight: FontWeight.w600, color: Colors.grey.shade500),
                hintText: 'How did it help? How long did you try it? Any tips for others?',
                hintStyle: TextStyle(fontSize: 13, color: Colors.grey.shade400),
                counterStyle: TextStyle(fontSize: 10, fontWeight: FontWeight.w600, color: Colors.grey.shade400),
                border: InputBorder.none,
                contentPadding: const EdgeInsets.symmetric(horizontal: 16, vertical: 14),
            ...
            child: Row(
              children: [
                Container(
                  ...
                  child: const Center(child: Text('4', style: TextStyle(fontSize: 12, fontWeight: FontWeight.w900, color: Colors.white))),
                ),
                const SizedBox(width: 10),
                const Text('How effective was it?', style: TextStyle(fontSize: 14, fontWeight: FontWeight.w700, color: Color(0xFF2C3E50))),
              ],
            ),
          ),
          const SizedBox(height: 14),
          Row(
            ...
            children: List.generate(5, (index) {
              ...
              return GestureDetector(
                onTap: () => setState(() => _testimonialRating = index + 1),
                child: AnimatedScale(
          ...
          if (_testimonialRating > 0)
            Center(
              child: Padding(
                padding: const EdgeInsets.only(top: 8),
                child: Text(
                  ["", "Not Helpful", "Slightly Helpful", "Helpful", "Very Helpful", "Life Changing!"][_testimonialRating],
              ...
              children: [
                Row(
                  children: [
                    Container(
                      width: 24, height: 24,
                      decoration: BoxDecoration(color: const Color(0xFF7C4DFF), borderRadius: BorderRadius.circular(8)),
                      child: const Center(child: Text('5', style: TextStyle(fontSize: 12, fontWeight: FontWeight.w900, color: Colors.white))),
                    ),
                    const SizedBox(width: 10),
                    const Text('Privacy & Consent', style: TextStyle(fontSize: 14, fontWeight: FontWeight.w700, color: Color(0xFF2C3E50))),
                  ],
                ),
                const SizedBox(height: 12),
                    ...
                    children: [
                      Row(
                        children: [
                          ...
                          const SizedBox(width: 8),
                          const Text('Consent Notice', style: TextStyle(fontSize: 12, fontWeight: FontWeight.w700, color: Color(0xFF2C3E50))),
                        ],
                      ),
                      const SizedBox(height: 8),
                      Text(
                        'Your testimonial may be displayed publicly to help other users discover effective techniques. By submitting, you acknowledge that:',
                        style: TextStyle(fontSize: 11, fontWeight: FontWeight.w500, color: Colors.grey.shade600, height: 1.5),
                      ),
                      const SizedBox(height: 8),
                      Row(
                        crossAxisAlignment: CrossAxisAlignment.start,
                        children: [
                          Icon(Icons.check_circle_outline_rounded, size: 14, color: Colors.grey.shade500),
                          const SizedBox(width: 8),
                          Expanded(
                            child: Text('Content may be reviewed and moderated before publication', style: TextStyle(fontSize: 10, fontWeight: FontWeight.w500, color: Colors.grey.shade500)),
                          ),
                        ],
                      ),
                      const SizedBox(height: 4),
                      Row(
                        ...
                        children: [
                          ...
                          const SizedBox(width: 8),
                          Expanded(
                            child: Text('You can request removal of your testimonial at any time', style: TextStyle(fontSize: 10, fontWeight: FontWeight.w500, color: Colors.grey.shade500)),
                          ),
                        ],
                      ),
                      const SizedBox(height: 4),
                      Row(
                        ...
                        children: [
                          ...
                          const SizedBox(width: 8),
                          Expanded(
                            child: Text('Your contact information will never be shared', style: TextStyle(fontSize: 10, fontWeight: FontWeight.w500, color: Colors.grey.shade500)),
                          ),
                        ],
                      ),
                    ],
                ...
                GestureDetector(
                  ...
                  child: Container(
                    ...
                    decoration: BoxDecoration(
                      color: _consentRead ? const Color(0xFF7C4DFF).withOpacity(0.08) : Colors.transparent,
                      borderRadius: BorderRadius.circular(10),
                      border: Border.all(color: _consentRead ? const Color(0xFF7C4DFF) : Colors.grey.shade300),
                    ),
                    child: Row(
                      children: [
                        Container(
                          width: 22, height: 22,
                          decoration: BoxDecoration(
                            color: _consentRead ? const Color(0xFF7C4DFF) : Colors.transparent,
                            borderRadius: BorderRadius.circular(6),
                            border: Border.all(color: _consentRead ? const Color(0xFF7C4DFF) : Colors.grey.shade400, width: 2),
                          ),
                          child: _consentRead ? const Icon(Icons.check, size: 14, color: Colors.white) : null,
                        ),
                        const SizedBox(width: 10),
                        Expanded(
                          child: Text(
                            'I have read and understand the consent notice',
                            style: TextStyle(fontSize: 12, fontWeight: FontWeight.w600, color: _consentRead ? const Color(0xFF7C4DFF) : Colors.grey.shade600),
                          ),
                        ),
                      ],
                    ),
                  ...
                  child: Column(
                    ...
                    children: [
                        ...
                        children: [
                          Expanded(
                            child: Column(
                              crossAxisAlignment: CrossAxisAlignment.start,
                              children: [
                                Text('Allow public display?', style: TextStyle(fontSize: 13, fontWeight: FontWeight.w700, color: Colors.grey.shade700)),
                                const SizedBox(height: 2),
                                Text('Uncheck to keep private (for internal use only)', style: TextStyle(fontSize: 10, fontWeight: FontWeight.w500, color: Colors.grey.shade500)),
                              ],
                            ),
                          ),
                          Switch(
                            value: _consentForPublic,
                            onChanged: (value) => setState(() => _consentForPublic = value),
                      ...
                      if (_consentForPublic) ...[
                        ...
                        ...[
                          {'value': 'anonymous', 'label': 'Anonymous', 'icon': Icons.visibility_off_rounded, 'desc': 'No name shown'},
                          {'value': 'first_name', 'label': 'First name only', 'icon': Icons.person_outline_rounded, 'desc': 'e.g., "Sarah"'},
                          {'value': 'full_name', 'label': 'Full name', 'icon': Icons.person_rounded, 'desc': 'e.g., "Sarah Johnson"'},
                        ].map((option) {
                          final isSelected = _displayNameType == option['value'];
                          return GestureDetector(
                            onTap: () => setState(() => _displayNameType = option['value'] as String),
                            child: AnimatedContainer(
                              duration: const Duration(milliseconds: 200),
                              margin: const EdgeInsets.only(bottom: 8),
                              padding: const EdgeInsets.all(12),
                              decoration: BoxDecoration(
                                color: isSelected ? const Color(0xFF7C4DFF).withOpacity(0.08) : Colors.grey.shade50,
                                borderRadius: BorderRadius.circular(10),
                                border: Border.all(color: isSelected ? const Color(0xFF7C4DFF) : Colors.grey.shade200, width: isSelected ? 1.5 : 1),
                              ),
                              child: Row(
                                children: [
                                  Icon(option['icon'] as IconData, size: 18, color: isSelected ? const Color(0xFF7C4DFF) : Colors.grey.shade400),
                                  const SizedBox(width: 10),
                                  Expanded(
                                    child: Column(
                                      crossAxisAlignment: CrossAxisAlignment.start,
                                      children: [
                                        Text(option['label'] as String, style: TextStyle(fontSize: 12, fontWeight: FontWeight.w700, color: isSelected ? const Color(0xFF7C4DFF) : Colors.grey.shade600)),
                                        Text(option['desc'] as String, style: TextStyle(fontSize: 10, fontWeight: FontWeight.w500, color: Colors.grey.shade400)),
                                      ],
                                    ),
                                  ),
                                  if (isSelected)
                                    Container(
                                      width: 20, height: 20,
                                      decoration: BoxDecoration(color: const Color(0xFF7C4DFF), shape: BoxShape.circle),
          ...
          SizedBox(
            ...
            child: ElevatedButton.icon(
              ...
              style: ElevatedButton.styleFrom(
                backgroundColor: kPrimaryGreen,
                elevation: 0,
                disabledBackgroundColor: kPrimaryGreen.withOpacity(0.4),
                shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(16)),
              ),
              icon: _isSubmittingTestimonial
                  ? const SizedBox(width: 20, height: 20, child: CircularProgressIndicator(strokeWidth: 2.5, color: Colors.white))
                  : const Icon(Icons.send_rounded, size: 20, color: Colors.white),
              label: Text(
                _isSubmittingTestimonial ? 'Submitting...' : 'Submit Testimonial',
          ...
          Container(
            ...
            child: Row(
              children: [
                ...
                Expanded(
                  child: Text(
                    'Your data is encrypted and protected. You can delete your testimonial anytime from Settings.',
  ...
  // ── DATABASE TESTIMONIAL CARD ──
  Widget _buildDbTestimonialCard(Map<String, dynamic> testimonial) {
    final sessionTypeData = _sessionTypes.firstWhere(
      (t) => t['value'] == testimonial['session_type'],
      orElse: () => {'value': 'General', 'icon': Icons.favorite_rounded, 'color': const Color(0xFFE91E63)},
    );

    final moodValue = testimonial['mood_when_it_worked'] ?? '';
    final moodCategory = kMoodCategories.firstWhere(
      (m) => m.value == moodValue,
      orElse: () => kMoodCategories[4],
    );

    String displayName = testimonial['user_name'] ?? 'Anonymous';
    final displayNameType = testimonial['display_name_type'] ?? 'anonymous';
    if (displayNameType == 'anonymous') {
      displayName = 'Anonymous';
    } else if (displayNameType == 'first_name' && displayName.isNotEmpty) {
    ...
    return Container(
      ...
      decoration: BoxDecoration(
        ...
        border: Border.all(color: moodCategory.color.withOpacity(0.15), width: 1),
      ),
      child: Column(
        crossAxisAlignment: CrossAxisAlignment.start,
        children: [
          Row(
            children: [
              Container(
                ...
                decoration: BoxDecoration(
                  color: moodCategory.color.withOpacity(0.12),
                  borderRadius: BorderRadius.circular(10),
                  border: Border.all(color: moodCategory.color.withOpacity(0.3)),
                ),
                child: Row(
                  mainAxisSize: MainAxisSize.min,
                  children: [
                    Text(moodCategory.emoji, style: const TextStyle(fontSize: 14)),
                    const SizedBox(width: 4),
                    Text(moodCategory.label, style: TextStyle(fontSize: 11, fontWeight: FontWeight.w700, color: moodCategory.color)),
                  ],
                ),
              ),
              const Spacer(),
              Row(
                children: List.generate(5, (i) => Icon(
                  i < (testimonial['star_rating'] ?? 5) ? Icons.star_rounded : Icons.star_outline_rounded,
                  size: 14,
                  color: i < (testimonial['star_rating'] ?? 5) ? const Color(0xFFFFD700) : Colors.grey.shade300,
                )),
              ),
            ],
          ),
          ...
          Row(
            children: [
              Container(
                width: 32, height: 32,
                decoration: BoxDecoration(
                  color: (sessionTypeData['color'] as Color).withOpacity(0.1),
                  borderRadius: BorderRadius.circular(10),
                ),
                child: Icon(sessionTypeData['icon'] as IconData, color: sessionTypeData['color'] as Color, size: 16),
              ),
              const SizedBox(width: 10),
              Expanded(
                child: Column(
                  ...
                  children: [
                    Row(
                      children: [
                        Text(displayName, style: const TextStyle(fontWeight: FontWeight.w800, fontSize: 14, color: Color(0xFF2C3E50))),
                        if (displayNameType == 'anonymous') ...[
                          const SizedBox(width: 6),
                          Icon(Icons.visibility_off_rounded, size: 12, color: Colors.grey.shade400),
                        ],
                      ],
                    ),
                    Text(testimonial['session_type'] ?? 'General', style: TextStyle(fontSize: 11, fontWeight: FontWeight.w600, color: sessionTypeData['color'] as Color)),
                  ],
                ),
              ),
            ],
          ...
          Container(
            ...
            child: Column(
              ...
              children: [
                Row(
                  children: [
                    Icon(Icons.check_circle_rounded, color: const Color(0xFF63EB68), size: 16),
                    const SizedBox(width: 6),
                    Text('What Worked', style: TextStyle(fontSize: 10, fontWeight: FontWeight.w800, color: const Color(0xFF63EB68).withOpacity(0.8), letterSpacing: 0.5)),
                  ],
                ),
                const SizedBox(height: 8),
                Text(
                  testimonial['what_worked'] ?? '',
                  style: const TextStyle(fontSize: 15, fontWeight: FontWeight.w700, color: Color(0xFF2C3E50), height: 1.4),
                ),
              ],
            ),
          ),
          if ((testimonial['description'] ?? '').isNotEmpty) ...[
            const SizedBox(height: 12),
            Container(
              padding: const EdgeInsets.all(12),
              decoration: BoxDecoration(
                ...
                children: [
                  ...
                  Expanded(
                    child: Text(
                      ...
                      style: TextStyle(fontSize: 13, fontWeight: FontWeight.w500, color: Colors.grey.shade600, height: 1.5, fontStyle: FontStyle.italic),
                    ),
                  ),
                ],
              ),
          ...
          Row(
            children: [
              GestureDetector(
                onTap: () async {
                  await RatingService.markHelpful(testimonial['id'] ?? 0);
                  _loadTestimonials();
                },
                child: Container(
                  ...
                  child: Row(
                    ...
                    children: [
                      ...
                      Text('Helpful (${testimonial['helpful_count'] ?? 0})', style: TextStyle(fontSize: 11, fontWeight: FontWeight.w600, color: Colors.grey.shade500)),
                    ],
                  ),
                ),
              ),
                ...
                onTap: () {
                  showDialog(
                    ...
                    builder: (ctx) => AlertDialog(
                      title: const Text('Report Testimonial'),
                      content: const Text('Is this testimonial inappropriate or misleading?'),
                      actions: [
                        TextButton(
    ...
    return GestureDetector(
      ...
      child: Container(
        ...
        child: Container(
          decoration: BoxDecoration(
            borderRadius: BorderRadius.circular(20),
            gradient: LinearGradient(colors: [Colors.black.withOpacity(0.1), Colors.black.withOpacity(0.6)], begin: Alignment.topCenter, end: Alignment.bottomCenter),
          ),
          alignment: Alignment.bottomLeft,
          ...
          children: [
            Stack(
              children: [
                ClipRRect(
                  borderRadius: BorderRadius.circular(14),
                  child: Image.network(track['image']!, width: 64, height: 64, fit: BoxFit.cover, errorBuilder: (_, __, ___) => Container(color: Colors.grey[300], width: 64, height: 64)),
                ),
                Positioned.fill(
                  child: Container(
                    decoration: BoxDecoration(color: Colors.black.withOpacity(0.4), borderRadius: BorderRadius.circular(14)),
                    child: const Center(child: Icon(Icons.play_arrow_rounded, color: Colors.white, size: 28)),
                  ),
                ),
              ],
            ),
            ...
            Expanded(
              child: Column(
                ...
                mainAxisAlignment: MainAxisAlignment.center,
                children: [
                  Text(track['title']!, style: const TextStyle(fontWeight: FontWeight.w800, fontSize: 15, color: Color(0xFF2C3E50))),
                  const SizedBox(height: 3),
                  Text(track['artist']!, style: TextStyle(fontWeight: FontWeight.w600, fontSize: 12, color: Colors.grey.shade500)),
                  const SizedBox(height: 5),
                  Row(
                    children: [
                      Icon(Icons.hd_rounded, size: 12, color: kPrimaryGreen),
                      const SizedBox(width: 3),
                      Text('High Quality', style: TextStyle(fontSize: 10, fontWeight: FontWeight.w700, color: kPrimaryGreen)),
                      const SizedBox(width: 8),
                      Icon(Icons.open_in_new_rounded, size: 10, color: Colors.grey.shade400),
                      const SizedBox(width: 3),
                      Text('YouTube', style: TextStyle(fontSize: 10, fontWeight: FontWeight.w600, color: Colors.grey.shade400)),
                    ],
                  ),
                ],
              ),
            ...
            Container(
              padding: const EdgeInsets.all(10),
              decoration: BoxDecoration(color: Colors.grey.shade100, shape: BoxShape.circle),
              child: Icon(Icons.open_in_browser_rounded, color: Colors.grey.shade600, size: 20),
            ),
          ],
        ),
      ),
...
class _InsightsScreenState extends State<InsightsScreen> with SingleTickerProviderStateMixin {
  ...
  final List<MoodDataPoint> _weeklyMood = [
    MoodDataPoint(day: 'Mon', value: 0.6, label: 'Good'),
    MoodDataPoint(day: 'Tue', value: 0.35, label: 'Low'),
    MoodDataPoint(day: 'Wed', value: 0.75, label: 'Great'),
    MoodDataPoint(day: 'Thu', value: 0.5, label: 'Okay'),
    MoodDataPoint(day: 'Fri', value: 0.85, label: 'Great'),
    MoodDataPoint(day: 'Sat', value: 0.9, label: 'Amazing'),
    MoodDataPoint(day: 'Sun', value: 0.7, label: 'Good'),
  ];

  final List<MoodDataPoint> _monthlyMood = [
    MoodDataPoint(day: 'W1', value: 0.45, label: 'Okay'),
    MoodDataPoint(day: 'W2', value: 0.6, label: 'Good'),
    MoodDataPoint(day: 'W3', value: 0.72, label: 'Great'),
    MoodDataPoint(day: 'W4', value: 0.68, label: 'Good'),
  ];

  final List<MoodDataPoint> _weeklySleep = [
    MoodDataPoint(day: 'Mon', value: 0.72, label: '5.8h'),
    MoodDataPoint(day: 'Tue', value: 0.55, label: '4.4h'),
    MoodDataPoint(day: 'Wed', value: 0.85, label: '6.8h'),
    MoodDataPoint(day: 'Thu', value: 0.65, label: '5.2h'),
    MoodDataPoint(day: 'Fri', value: 0.9, label: '7.2h'),
    MoodDataPoint(day: 'Sat', value: 0.95, label: '7.6h'),
    MoodDataPoint(day: 'Sun', value: 0.78, label: '6.2h'),
  ];

  final List<SessionStat> _sessionStats = [
    SessionStat(label: 'Meditation', completed: 12, total: 14, color: const Color(0xFF5CC6A9)),
    SessionStat(label: 'Focus', completed: 8, total: 10, color: const Color(0xFF63EB68)),
    SessionStat(label: 'Sleep', completed: 5, total: 7, color: const Color(0xFF5C7CFA)),
    SessionStat(label: 'Fitness', completed: 3, total: 5, color: const Color(0xFFFF922B)),
  ];

  final List<SuggestionItem> _suggestions = [
    SuggestionItem(title: 'Your stress peaks on Tuesdays', description: 'Try a 10-minute breathing session every Tuesday morning to build resilience before the week intensifies.', icon: Icons.analytics_rounded, color: const Color(0xFFFF6B6B), tag: 'Pattern Detected'),
    SuggestionItem(title: 'Sleep improving — keep it up!', description: 'Your average sleep jumped from 5.1h to 6.3h this week. Maintaining your bedtime routine is working.', icon: Icons.trending_up_rounded, color: const Color(0xFF5CC6A9), tag: 'Positive Trend'),
    SuggestionItem(title: "You haven't tried Fitness yet", description: 'Just 1 mindful fitness session per week can boost your mood scores by up to 15%. Start with a gentle stretch.', icon: Icons.directions_run_rounded, color: const Color(0xFFFF922B), tag: 'Suggestion'),
    SuggestionItem(title: 'Weekend mood is consistently high', description: 'You feel best on Saturdays. Consider scheduling creative activities or social time to carry this into weekdays.', icon: Icons.emoji_emotions_rounded, color: const Color(0xFF748FFC), tag: 'Insight'),
  ];

  double get _avgMood {
    final data = _selectedPeriod == 0 ? _weeklyMood : _monthlyMood;
    return data.map((e) => e.value).reduce((a, b) => a + b) / data.length;
  }

  double get _avgSleep {
    return _weeklySleep.map((e) => e.value).reduce((a, b) => a + b) / _weeklySleep.length;
  }
        ...
        child: Column(
          children: [
            Padding(
              ...
              child: Row(
                children: [
                  const Text('Your Journey', style: TextStyle(fontSize: 28, fontWeight: FontWeight.w900, color: Color(0xFF2C3E50))),
                  const Spacer(),
                  Container(
                    ...
                    child: Row(
                      children: ['Week', 'Month'].asMap().entries.map((e) {
                        final active = _selectedPeriod == e.key;
                        return GestureDetector(
                          onTap: () => setState(() => _selectedPeriod = e.key),
                          child: AnimatedContainer(
                            duration: const Duration(milliseconds: 250),
                            padding: const EdgeInsets.symmetric(horizontal: 14, vertical: 7),
                            decoration: BoxDecoration(color: active ? kPrimaryGreen : Colors.transparent, borderRadius: BorderRadius.circular(10)),
                            child: Text(e.value, style: TextStyle(fontSize: 12, fontWeight: FontWeight.w800, color: active ? Colors.white : Colors.grey.shade500)),
                          ),
                        );
                      }).toList(),
                    ),
            ...
            Container(
              ...
              child: TabBar(
                ...
                indicator: BoxDecoration(color: Colors.white, borderRadius: BorderRadius.circular(12), boxShadow: [BoxShadow(color: Colors.black.withOpacity(0.06), blurRadius: 8, offset: const Offset(0, 2))]),
                ...
                tabs: const [Tab(text: 'Mood'), Tab(text: 'Activity'), Tab(text: 'Insights')],
              ),
            ),
            const SizedBox(height: 16),
            Expanded(
              child: TabBarView(controller: _tabController, children: [_buildMoodTab(), _buildActivityTab(), _buildInsightsTab()]),
            ),
          ],
        ),
      ),
      ...
      child: Column(
        ...
        children: [
          Container(
            ...
            decoration: BoxDecoration(gradient: const LinearGradient(colors: [Color(0xFF5CC6A9), Color(0xFF3DA88A)], begin: Alignment.topLeft, end: Alignment.bottomRight), borderRadius: BorderRadius.circular(24), boxShadow: [BoxShadow(color: const Color(0xFF5CC6A9).withOpacity(0.3), blurRadius: 20, offset: const Offset(0, 10))]),
            child: Row(
              children: [
                Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [const Text('Current Streak', style: TextStyle(fontSize: 13, color: Colors.white, fontWeight: FontWeight.w600)), const SizedBox(height: 4), const Row(crossAxisAlignment: CrossAxisAlignment.end, children: [Text('5', style: TextStyle(fontSize: 42, color: Colors.white, fontWeight: FontWeight.w900, height: 1.0)), SizedBox(width: 6), Padding(padding: EdgeInsets.only(bottom: 8), child: Text('days', style: TextStyle(fontSize: 16, color: Colors.white70, fontWeight: FontWeight.w500)))])]) ),
                Container(width: 70, height: 70, decoration: BoxDecoration(color: Colors.white.withOpacity(0.2), shape: BoxShape.circle, border: Border.all(color: Colors.white.withOpacity(0.3), width: 2)), child: Column(mainAxisAlignment: MainAxisAlignment.center, children: [Text('${(_avgMood * 100).round()}%', style: const TextStyle(color: Colors.white, fontSize: 18, fontWeight: FontWeight.w900)), Text('avg', style: TextStyle(color: Colors.white.withOpacity(0.7), fontSize: 10, fontWeight: FontWeight.w600))])),
              ],
            ),
          ),
          const SizedBox(height: 28),
          Row(children: [Container(padding: const EdgeInsets.symmetric(horizontal: 12, vertical: 8), decoration: BoxDecoration(color: moodColors[moodIdx].withOpacity(0.15), borderRadius: BorderRadius.circular(12), border: Border.all(color: moodColors[moodIdx].withOpacity(0.3))), child: Row(mainAxisSize: MainAxisSize.min, children: [Icon(Icons.mood_rounded, color: moodColors[moodIdx], size: 18), const SizedBox(width: 6), Text('Avg: ${moodLabels[moodIdx]}', style: TextStyle(fontSize: 13, fontWeight: FontWeight.w800, color: moodColors[moodIdx]))])), const Spacer(), Text(_selectedPeriod == 0 ? 'This week' : 'This month', style: const TextStyle(fontSize: 13, fontWeight: FontWeight.w600, color: Colors.grey))]),
          const SizedBox(height: 20),
...
class _MoodBarChartPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    final maxH = size.height - 30;
    final baseY = size.height - 20;
    for (int i = 0; i <= 4; i++) {
      final y = baseY - (maxH * i / 4);
      canvas.drawLine(Offset(0, y), Offset(size.width, y), Paint()..color = Colors.grey.shade100..strokeWidth = 1);
      final labels = ['0', '25', '50', '75', '100'];
      final tp = TextPainter(text: TextSpan(text: labels[i], style: TextStyle(fontSize: 9, color: Colors.grey.shade400, fontWeight: FontWeight.w600)), textDirection: TextDirection.ltr)..layout();
      tp.paint(canvas, Offset(0, y - 12));
    }
    for (int i = 0; i < count; i++) {
      final x = (size.width / count) * i + (size.width / count - barWidth) / 2;
      final h = maxH * data[i].value;
      final y = baseY - h;
      final r = Radius.circular(barWidth / 2.0);
      canvas.drawRRect(RRect.fromRectAndCorners(Rect.fromLTWH(x, baseY - maxH, barWidth, maxH), topRight: r, topLeft: r), Paint()..color = backgroundColor);
      final rect = Rect.fromLTWH(x, y, barWidth, h);
      final gradient = LinearGradient(begin: Alignment.bottomCenter, end: Alignment.topCenter, colors: [barColor.withOpacity(0.6), barColor]);
      canvas.drawRRect(RRect.fromRectAndCorners(rect, topRight: r, topLeft: r), Paint()..shader = gradient.createShader(rect));
      final valPaint = TextPainter(text: TextSpan(text: '${(data[i].value * 100).round()}', style: TextStyle(fontSize: 9, fontWeight: FontWeight.w800, color: barColor)), textDirection: TextDirection.ltr)..layout();
      valPaint.paint(canvas, Offset(x + barWidth / 2 - valPaint.width / 2, y - 14));
      final dayPaint = TextPainter(text: TextSpan(text: data[i].day, style: const TextStyle(fontSize: 10, fontWeight: FontWeight.w700, color: Color(0xFF9CA3AF))), textDirection: TextDirection.ltr)..layout();
      dayPaint.paint(canvas, Offset(x + barWidth / 2 - dayPaint.width / 2, baseY + 4));
    }
  }
...
class _SleepLineChartPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    final maxH = size.height - 30;
    final baseY = size.height - 20;
    for (int i = 0; i <= 4; i++) { final y = baseY - (maxH * i / 4); canvas.drawLine(Offset(padding, y), Offset(size.width - padding, y), Paint()..color = Colors.grey.shade100..strokeWidth = 1); }
    final goalY = baseY - maxH * goalLine;
    final goalPath = Path()..moveTo(padding, goalY)..lineTo(size.width - padding, goalY);
    canvas.drawPath(dashPath(goalPath, dashArray: CircularIntervalList<double>(const [6.0, 4.0])), Paint()..color = const Color(0xFFFF6B6B)..strokeWidth = 1.5..style = PaintingStyle.stroke);
    final points = <Offset>[];
    for (int i = 0; i < data.length; i++) { final x = padding + (chartW / (data.length - 1)) * i; final y = baseY - maxH * data[i].value; points.add(Offset(x, y)); }
    final fillPath = Path()..moveTo(points.first.dx, baseY)..lineTo(points.first.dx, points.first.dy);
    for (int i = 1; i < points.length; i++) { final cp1x = (points[i - 1].dx + points[i].dx) / 2; final cp1y = points[i - 1].dy; final cp2x = (points[i - 1].dx + points[i].dx) / 2; final cp2y = points[i].dy; fillPath.cubicTo(cp1x, cp1y, cp2x, cp2y, points[i].dx, points[i].dy); }
    fillPath.lineTo(points.last.dx, baseY); fillPath.close();
    canvas.drawPath(fillPath, Paint()..shader = LinearGradient(begin: Alignment.topCenter, end: Alignment.bottomCenter, colors: [fillColor.withOpacity(0.15), fillColor.withOpacity(0.02)]).createShader(Rect.fromLTWH(0.0, 0.0, size.width, size.height)));
    final linePath = Path()..moveTo(points.first.dx, points.first.dy);
    for (int i = 1; i < points.length; i++) { final cp1x = (points[i - 1].dx + points[i].dx) / 2; final cp1y = points[i - 1].dy; final cp2x = (points[i - 1].dx + points[i].dx) / 2; final cp2y = points[i].dy; linePath.cubicTo(cp1x, cp1y, cp2x, cp2y, points[i].dx, points[i].dy); }
    canvas.drawPath(linePath, Paint()..color = lineColor..strokeWidth = 2.5..style = PaintingStyle.stroke..strokeCap = StrokeCap.round);
    for (int i = 0; i < points.length; i++) {
      canvas.drawCircle(points[i], 5, Paint()..color = Colors.white);
      canvas.drawCircle(points[i], 3.5, Paint()..color = lineColor);
      final labelPaint = TextPainter(text: TextSpan(text: data[i].label, style: TextStyle(fontSize: 9, fontWeight: FontWeight.w800, color: lineColor)), textDirection: TextDirection.ltr)..layout();
      labelPaint.paint(canvas, Offset(points[i].dx - labelPaint.width / 2, points[i].dy - 16));
      final dayPaint = TextPainter(text: TextSpan(text: data[i].day, style: const TextStyle(fontSize: 10, fontWeight: FontWeight.w700, color: Color(0xFF9CA3AF))), textDirection: TextDirection.ltr)..layout();
      dayPaint.paint(canvas, Offset(points[i].dx - dayPaint.width / 2, baseY + 4));
    }
  }
...
class _CircularProgressPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    canvas.drawArc(Rect.fromCircle(center: center, radius: radius), -math.pi / 2, 2 * math.pi * progress, false, Paint()..color = color..style = PaintingStyle.stroke..strokeWidth = strokeWidth..strokeCap = StrokeCap.round);
  }
