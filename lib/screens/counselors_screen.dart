...
class CounselorAppointment {
  ...
  String get formattedDate {
    ...
    if (dateOnly.year == tomorrow.year && dateOnly.month == tomorrow.month && dateOnly.day == tomorrow.day) {
      return 'Tomorrow';
    }
...
class _CounselorsScreenState extends State<CounselorsScreen> {
  ...
  void initState() {
    super.initState();
    _loadUserRoleAndInitialize();
  }

  // 🔒 SECURITY: Load role with robust detection and debug logging
  Future<void> _loadUserRoleAndInitialize() async {
    final auth = AuthService();

    // First, try to get the freshest role from the /user endpoint
    try {
      final userData = await auth.getUser();
      // The getUser method in AuthService already saves role/name/email
      // Now read what was saved
    } catch (e) {
      print('⚠️ Could not fetch fresh user data: $e');
    }
    ...
    print('🔑 ====== ROLE DEBUG ======');
    print('🔑 Raw role from storage: "$role"');
    print('🔑 User name: "$name"');
    print('🔑 User email: "$email"');
    print('🔑 Is counselor (exact): ${role == 'counselor'}');
    print('🔑 Is counselor (lowercase): ${role.toLowerCase() == 'counselor'}');
    print('🔑 Is admin (lowercase): ${role.toLowerCase() == 'admin'}');
    print('🔑 canAccessCounselorFeatures: ${role.toLowerCase() == 'counselor' || role.toLowerCase() == 'admin'}');
    print('🔑 =========================');

    setState(() {
      _userRole = role.toLowerCase().trim(); // Normalize to lowercase
      _userName = name;
      _userEmail = email;
      _isRoleLoading = false;
    });

    print('🔑 _userRole after setState: "$_userRole"');
    print('🔑 _canAccessCounselorFeatures: $_canAccessCounselorFeatures');

    _fetchCounselorsFromApi();
    _loadUserAppointments();

    if (_canAccessCounselorFeatures) {
      _loadCounselorAppointments();
    }
  }

  // 🔒 SECURITY: Case-insensitive role check
  bool get _canAccessCounselorFeatures {
  ...
  Future<void> _loadUserAppointments() async {
    ...
    try {
      final appointments = await AuthService().fetchUserAppointments();
      setState(() {
      ...
      setState(() {
        _userAppointmentsError = 'Failed to load your appointments. Please try again.';
        _isUserAppointmentsLoading = false;
      });
    }
  }
  ...
  Future<void> _loadCounselorAppointments() async {
    if (!_canAccessCounselorFeatures) return;

    setState(() => _isAppointmentsLoading = true);
    try {
      final appointments = await AuthService().fetchCounselorAppointments();
      setState(() {
  ...
  Future<void> _fetchCounselorsFromApi() async {
    ...
    try {
      final data = await AuthService().fetchCounselors();
      setState(() {
  ...
  // ────────────────────────────────────────────────
  //  APPOINTMENT SUBMISSION (Everyone)
  // ────────────────────────────────────────────────
      ...
      builder: (ctx) => Dialog(
        ...
        child: Padding(
          ...
          child: Column(
            ...
            children: [
              Container(width: 80, height: 80, decoration: BoxDecoration(color: kPrimaryGreen.withOpacity(0.1), shape: BoxShape.circle), child: const Icon(Icons.check_circle_rounded, color: kPrimaryGreen, size: 48)),
              ...
              Text(
                counselor != null ? 'Your appointment request with ${counselor.name} has been submitted successfully.' : 'Your appointment request has been submitted. An admin will assign a counselor to you shortly.',
              ...
              Row(
                children: [
                  Expanded(child: GestureDetector(onTap: () => Navigator.pop(ctx), child: Container(padding: const EdgeInsets.symmetric(vertical: 14), decoration: BoxDecoration(color: Colors.grey.shade100, borderRadius: BorderRadius.circular(14)), child: const Text('Close', textAlign: TextAlign.center, style: TextStyle(fontSize: 15, fontWeight: FontWeight.w700, color: Colors.grey))))),
                  const SizedBox(width: 12),
                  Expanded(child: GestureDetector(onTap: () { Navigator.pop(ctx); _showMyAppointmentsSheet(); }, child: Container(padding: const EdgeInsets.symmetric(vertical: 14), decoration: BoxDecoration(color: kPrimaryGreen, borderRadius: BorderRadius.circular(14), boxShadow: [BoxShadow(color: kPrimaryGreen.withOpacity(0.3), blurRadius: 10, offset: const Offset(0, 4))]), child: const Text('View My Requests', textAlign: TextAlign.center, style: TextStyle(fontSize: 15, fontWeight: FontWeight.w700, color: Colors.white))))),
                ],
              ),
            ],
          ),
  ...
  void _acceptAppointment(String appointmentId) async {
    if (!_canAccessCounselorFeatures) { _showUnauthorizedSnackBar(); return; }
    try {
      final success = await AuthService().updateAppointmentStatus(appointmentId, 'confirmed');
      if (success) {
        setState(() {
        ...
        actions: [
          ...
          TextButton(
            onPressed: () async {
              Navigator.pop(ctx);
              try {
                final success = await AuthService().updateAppointmentStatus(appointmentId, 'cancelled');
                if (success) {
  ...
  Future<void> _submitCounselorSessionLog() async {
    ...
    try {
      final success = await AuthService().submitCounselorSessionLog(
  ...
  void _showUnauthorizedSnackBar() {
    ScaffoldMessenger.of(context).showSnackBar(
      const SnackBar(content: Text('You do not have permission to perform this action.'), backgroundColor: Colors.red, behavior: SnackBarBehavior.floating),
    );
  }
                ...
                children: [
                  ...
                  Expanded(
                    child: _isAppointmentsLoading
                        ...
                        : _counselorAppointments.isEmpty
                            ? _buildNoAppointmentsState()
                            : ListView.builder(
                                padding: const EdgeInsets.symmetric(horizontal: 16, vertical: 12),
                                itemCount: _counselorAppointments.length,
                                itemBuilder: (context, index) => _buildAppointmentCard(_counselorAppointments[index], setSheetState),
                              ),
                  ),
                ],
              ),
  ...
  void _showCounselorSessionLogSheet() {
    if (!_canAccessCounselorFeatures) { _showUnauthorizedSnackBar(); return; }
    if (_counselorNameController.text.isEmpty) _counselorNameController.text = _userName;
    if (_counselorEmailController.text.isEmpty) _counselorEmailController.text = _userEmail;
          ...
          child: SingleChildScrollView(
            ...
            child: Column(
              ...
              children: [
                ...
                const SizedBox(height: 24),
                Form(
                  key: _counselorFormKey,
                  child: Column(
                    crossAxisAlignment: CrossAxisAlignment.start,
                    children: [
                      _buildCounselorSectionLabel('Your Details', Icons.person_rounded, const Color(0xFF7C3AED)),
                      const SizedBox(height: 12),
                      _buildCounselorFormLabel('Full Name *'),
                      const SizedBox(height: 8),
                      _buildCounselorFormField(controller: _counselorNameController, hint: 'Enter your full name', icon: Icons.badge_rounded, readOnly: true, validator: (v) => (v == null || v.trim().isEmpty) ? 'Name is required' : null),
                      const SizedBox(height: 16),
                      _buildCounselorFormLabel('Email Address *'),
                      const SizedBox(height: 8),
                      _buildCounselorFormField(controller: _counselorEmailController, hint: 'your@email.com', icon: Icons.email_rounded, readOnly: true, validator: (v) { if (v == null || v.trim().isEmpty) return 'Email is required'; if (!v.contains('@')) return 'Enter a valid email'; return null; }),
                      const SizedBox(height: 16),
                      _buildCounselorFormLabel('Contact Number *'),
                      const SizedBox(height: 8),
                      ...
                      const SizedBox(height: 24),
                      _buildCounselorSectionLabel('Session Details', Icons.assignment_rounded, const Color(0xFF0369A1)),
                      const SizedBox(height: 12),
                      _buildCounselorFormLabel('Client / Patient Name *'),
                      const SizedBox(height: 8),
                      ...
                      const SizedBox(height: 16),
                      _buildCounselorFormLabel('Specification / Category *'),
                      const SizedBox(height: 8),
                      ...
                      const SizedBox(height: 16),
                      _buildCounselorFormLabel('Session Notes'),
                      const SizedBox(height: 8),
                      _buildCounselorFormField(controller: _sessionNotesController, hint: 'Brief summary...', icon: Icons.notes_rounded, maxLines: 3),
                      const SizedBox(height: 24),
                      _buildCounselorSectionLabel('Session Screenshot *', Icons.screenshot_rounded, const Color(0xFF16A34A)),
                      const SizedBox(height: 4),
                      Text('Upload proof of the completed session', style: TextStyle(fontSize: 11, fontWeight: FontWeight.w500, color: Colors.grey.shade500)),
                      const SizedBox(height: 12),
                      if (_screenshotFile != null) _buildScreenshotPreview() else _buildScreenshotUploadArea(),
                      const SizedBox(height: 32),
                      GestureDetector(
                        onTap: _isCounselorSubmitting ? null : _submitCounselorSessionLog,
                        child: AnimatedContainer(
                          duration: const Duration(milliseconds: 200),
                          width: double.infinity,
                          padding: const EdgeInsets.symmetric(vertical: 18),
                          decoration: BoxDecoration(
                            gradient: _isCounselorSubmitting
                            ...
                            boxShadow: _isCounselorSubmitting ? [] : [BoxShadow(color: const Color(0xFF7C3AED).withOpacity(0.4), blurRadius: 15, offset: const Offset(0, 8))],
                          ),
                          child: Center(
                            child: _isCounselorSubmitting
                                ? const SizedBox(width: 22, height: 22, child: CircularProgressIndicator(color: Colors.white, strokeWidth: 2.5))
                                : const Row(mainAxisAlignment: MainAxisAlignment.center, children: [Icon(Icons.upload_rounded, color: Colors.white, size: 20), SizedBox(width: 8), Text('Submit Session Log', style: TextStyle(color: Colors.white, fontSize: 16, fontWeight: FontWeight.w900))]),
                          ),
                        ),
                      ),
                      const SizedBox(height: 20),
  ...
  // ────────────────────────────────────────────────
  //  ALL USERS: My Appointments Sheet
  // ────────────────────────────────────────────────
          ...
          child: ClipRRect(
            ...
            child: Column(
              children: [
                Container(
                  padding: const EdgeInsets.all(20),
                  decoration: const BoxDecoration(color: kPrimaryGreen, borderRadius: BorderRadius.vertical(top: Radius.circular(28))),
                  child: Column(
                    children: [
                      Center(child: Container(width: 40, height: 4, decoration: BoxDecoration(color: Colors.white.withOpacity(0.4), borderRadius: BorderRadius.circular(4)))),
                      const SizedBox(height: 16),
                      Row(children: [
                        ...
                        const SizedBox(width: 12),
                        const Text("My Appointment Requests", style: TextStyle(fontSize: 20, fontWeight: FontWeight.w900, color: Colors.white)),
                        const Spacer(),
                        GestureDetector(
                          onTap: () { Navigator.pop(context); _loadUserAppointments(); },
                          child: Container(padding: const EdgeInsets.all(8), decoration: BoxDecoration(color: Colors.white.withOpacity(0.2), borderRadius: BorderRadius.circular(10)), child: const Icon(Icons.refresh_rounded, color: Colors.white, size: 18)),
                        ),
                      ]),
                      const SizedBox(height: 4),
                      Text('${_userAppointments.length} total requests', style: TextStyle(fontSize: 13, fontWeight: FontWeight.w600, color: Colors.white.withOpacity(0.8))),
                    ],
                  ),
                ),
                Expanded(
                  child: _isUserAppointmentsLoading
                      ...
                      : _userAppointments.isEmpty
                          ? _buildNoUserAppointments()
                          : ListView.builder(padding: const EdgeInsets.all(16), itemCount: _userAppointments.length, itemBuilder: (context, index) => _buildUserAppointmentCard(_userAppointments[index])),
                ),
              ],
            ),
          ),
  ...
  Widget _buildNoUserAppointments() {
    return Center(
      child: Padding(
        ...
        child: Column(
          children: [
            Container(padding: const EdgeInsets.all(20), decoration: BoxDecoration(color: kPrimaryGreen.withOpacity(0.1), shape: BoxShape.circle), child: const Icon(Icons.event_available_rounded, size: 48, color: kPrimaryGreen)),
            const SizedBox(height: 20),
            const Text('No Appointments Yet', style: TextStyle(fontSize: 18, fontWeight: FontWeight.w800, color: Colors.black87)),
            const SizedBox(height: 8),
            const Text('Submit your first appointment request below!', style: TextStyle(fontSize: 13, fontWeight: FontWeight.w500, color: Colors.grey)),
            if (_userAppointmentsError != null) ...[
              const SizedBox(height: 20),
              Container(
                ...
                child: Row(children: [
                  ...
                  Expanded(child: Text(_userAppointmentsError!, style: const TextStyle(fontSize: 12, fontWeight: FontWeight.w500, color: Color(0xFF991B1B), height: 1.4))),
                  GestureDetector(onTap: _loadUserAppointments, child: Container(padding: const EdgeInsets.symmetric(horizontal: 12, vertical: 6), decoration: BoxDecoration(color: const Color(0xFFDC2626), borderRadius: BorderRadius.circular(8)), child: const Text('Retry', style: TextStyle(color: Colors.white, fontSize: 11, fontWeight: FontWeight.w800)))),
                ]),
              ),
            ],
          ],
        ...
        child: Column(
          ...
          children: [
            Row(
              ...
              children: [
                Container(padding: const EdgeInsets.symmetric(horizontal: 10, vertical: 5), decoration: BoxDecoration(color: isPending ? const Color(0xFFF59E0B) : kPrimaryGreen, borderRadius: BorderRadius.circular(8)), child: Text(isPending ? 'Pending' : 'Confirmed', style: const TextStyle(fontSize: 11, fontWeight: FontWeight.w800, color: Colors.white))),
                Text(DateFormat('MMM dd').format(appointmentDate), style: TextStyle(fontSize: 12, fontWeight: FontWeight.w600, color: Colors.grey.shade500)),
              ],
            ),
            const SizedBox(height: 12),
            Text(appointment['service'] ?? '', style: const TextStyle(fontSize: 15, fontWeight: FontWeight.w700, color: Colors.black87)),
            const SizedBox(height: 8),
            Row(children: [const Icon(Icons.access_time_rounded, size: 16, color: Colors.grey), const SizedBox(width: 6), Text(_formatTime12Hour(appointment['appointmentTime'] ?? appointment['appointment_time'] ?? ''), style: const TextStyle(fontSize: 13, fontWeight: FontWeight.w600, color: Colors.grey))]),
            if (appointment['notes'] != null && appointment['notes'].toString().isNotEmpty) ...[
              const SizedBox(height: 8),
              Text(appointment['notes'], style: TextStyle(fontSize: 12, fontWeight: FontWeight.w500, color: Colors.grey.shade500, height: 1.4), maxLines: 2, overflow: TextOverflow.ellipsis),
            ],
          ],
        ),
      ),
    ...
    return Container(
      ...
      child: Padding(
        ...
        child: Column(
          ...
          children: [
            Row(children: [
              Container(padding: const EdgeInsets.symmetric(horizontal: 10, vertical: 6), decoration: BoxDecoration(color: isPending ? const Color(0xFFF59E0B) : kPrimaryGreen, borderRadius: BorderRadius.circular(8)), child: Row(mainAxisSize: MainAxisSize.min, children: [Icon(isPending ? Icons.schedule_rounded : Icons.check_circle_rounded, color: Colors.white, size: 12), const SizedBox(width: 4), Text(_formatTime12Hour(appointment.appointmentTime), style: const TextStyle(fontSize: 11, fontWeight: FontWeight.w800, color: Colors.white))])),
              const Spacer(),
              _buildStatusBadge(appointment.status),
            ]),
            const SizedBox(height: 14),
            Row(children: [
              Container(padding: const EdgeInsets.all(3), decoration: BoxDecoration(shape: BoxShape.circle, border: Border.all(color: const Color(0xFF7C3AED), width: 2)), child: CircleAvatar(radius: 22, backgroundColor: const Color(0xFF7C3AED).withOpacity(0.1), child: Text(appointment.patientName.split(' ').map((n) => n[0]).take(2).join(), style: const TextStyle(fontSize: 14, fontWeight: FontWeight.w800, color: Color(0xFF7C3AED))))),
              const SizedBox(width: 12),
              Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [Text(appointment.patientName, style: const TextStyle(fontSize: 15, fontWeight: FontWeight.w800, color: Colors.black87)), const SizedBox(height: 2), Text(appointment.service, style: const TextStyle(color: Colors.grey))])),
            ]),
            const SizedBox(height: 14),
            Container(
  ...
  Widget _buildCounselorFormField({required TextEditingController controller, required String hint, required IconData icon, String? Function(String?)? validator, int maxLines = 1, bool readOnly = false}) {
    return TextFormField(
      ...
      decoration: InputDecoration(
        hintText: hint,
        prefixIcon: Icon(icon, color: readOnly ? Colors.grey.shade300 : Colors.grey.shade400, size: 20),
        ...
        enabledBorder: OutlineInputBorder(borderRadius: BorderRadius.circular(14), borderSide: BorderSide(color: readOnly ? Colors.grey.shade300 : Colors.grey.shade200)),
        focusedBorder: OutlineInputBorder(borderRadius: BorderRadius.circular(14), borderSide: BorderSide(color: readOnly ? Colors.grey.shade300 : const Color(0xFF7C3AED), width: readOnly ? 1 : 2)),
  ...
  Widget _buildScreenshotUploadArea() {
    return Container(
      ...
      decoration: BoxDecoration(color: const Color(0xFFF8F9FA), borderRadius: BorderRadius.circular(18), border: Border.all(color: Colors.grey.shade200, width: 2, strokeAlign: BorderSide.strokeAlignOutside)),
      child: Column(children: [
        Container(padding: const EdgeInsets.all(14), decoration: BoxDecoration(color: const Color(0xFF16A34A).withOpacity(0.1), shape: BoxShape.circle), child: const Icon(Icons.cloud_upload_rounded, color: Color(0xFF16A34A), size: 32)),
        const SizedBox(height: 14),
        const Text('Tap to upload screenshot', style: TextStyle(fontSize: 14, fontWeight: FontWeight.w800, color: Colors.black87)),
        const SizedBox(height: 4),
        Text('PNG, JPG up to 5MB', style: TextStyle(fontSize: 12, fontWeight: FontWeight.w500, color: Colors.grey.shade500)),
        const SizedBox(height: 16),
        Row(mainAxisAlignment: MainAxisAlignment.center, children: [
          _buildImageSourceButton(label: 'Camera', icon: Icons.camera_alt_rounded, onTap: () => _pickScreenshot(ImageSource.camera)),
          const SizedBox(width: 12),
          _buildImageSourceButton(label: 'Gallery', icon: Icons.photo_library_rounded, onTap: () => _pickScreenshot(ImageSource.gallery)),
        ]),
      ]),
    );
  }
  ...
  Widget _buildScreenshotPreview() {
    return Container(
      width: double.infinity,
      decoration: BoxDecoration(color: const Color(0xFFF8F9FA), borderRadius: BorderRadius.circular(18), border: Border.all(color: const Color(0xFF16A34A), width: 2)),
      child: Column(children: [
        ClipRRect(borderRadius: const BorderRadius.vertical(top: Radius.circular(16)), child: Image.file(_screenshotFile!, height: 200, width: double.infinity, fit: BoxFit.cover)),
        Padding(padding: const EdgeInsets.all(14), child: Row(children: [
          Container(padding: const EdgeInsets.all(6), decoration: BoxDecoration(color: const Color(0xFF16A34A).withOpacity(0.1), shape: BoxShape.circle), child: const Icon(Icons.check_circle_rounded, color: Color(0xFF16A34A), size: 18)),
          const SizedBox(width: 10),
          Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [Text('Screenshot uploaded', style: TextStyle(fontSize: 13, fontWeight: FontWeight.w800, color: Colors.grey.shade700)), Text(_screenshotFile!.path.split('/').last, style: TextStyle(fontSize: 11, fontWeight: FontWeight.w500, color: Colors.grey.shade400), overflow: TextOverflow.ellipsis)])),
          const SizedBox(width: 8),
          GestureDetector(onTap: _removeScreenshot, child: Container(padding: const EdgeInsets.all(8), decoration: BoxDecoration(color: Colors.red.shade50, borderRadius: BorderRadius.circular(10)), child: Icon(Icons.delete_outline_rounded, color: Colors.red.shade400, size: 20))),
        ])),
      ]),
    );
  }
  ...
  // ────────────────────────────────────────────────
  //  PATIENT-SPECIFIC BUILDERS (Visible to Everyone)
  // ────────────────────────────────────────────────
  ...
  Widget _shimmerCircle({required double radius}) => Container(width: radius * 2, height: radius * 2, decoration: const BoxDecoration(color: Colors.grey, shape: BoxShape.circle));

  Widget _buildNoAllocationPlaceholder() {
    return Container(
      padding: const EdgeInsets.all(24),
      decoration: BoxDecoration(color: Colors.white, borderRadius: BorderRadius.circular(24), border: Border.all(color: Colors.grey.shade200), boxShadow: [BoxShadow(color: Colors.black.withOpacity(0.03), blurRadius: 12, offset: const Offset(0, 6))]),
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        Container(padding: const EdgeInsets.symmetric(horizontal: 12, vertical: 6), decoration: BoxDecoration(color: Colors.grey.shade100, borderRadius: BorderRadius.circular(8)), child: const Row(mainAxisSize: MainAxisSize.min, children: [Icon(Icons.hourglass_top_rounded, color: Colors.grey, size: 14), SizedBox(width: 6), Text('Awaiting Assignment', style: TextStyle(color: Colors.grey, fontWeight: FontWeight.w800, fontSize: 12))])),
        const SizedBox(height: 18),
        const Text('Your counselor is being assigned', style: TextStyle(fontSize: 18, fontWeight: FontWeight.w900, color: Colors.black87)),
        const SizedBox(height: 8),
        Text("An admin is reviewing your profile and will assign a counselor shortly. You'll be notified once assigned.", style: TextStyle(fontSize: 13, fontWeight: FontWeight.w500, color: Colors.grey.shade500, height: 1.6)),
        const SizedBox(height: 20),
        GestureDetector(onTap: _scrollToForm, child: Container(width: double.infinity, padding: const EdgeInsets.symmetric(vertical: 14), decoration: BoxDecoration(color: kPrimaryGreen, borderRadius: BorderRadius.circular(14), boxShadow: [BoxShadow(color: kPrimaryGreen.withOpacity(0.3), blurRadius: 10, offset: const Offset(0, 4))]), child: const Row(mainAxisAlignment: MainAxisAlignment.center, children: [Icon(Icons.arrow_downward_rounded, color: Colors.white, size: 18), SizedBox(width: 8), Text('Book Appointment Now', style: TextStyle(color: Colors.white, fontSize: 15, fontWeight: FontWeight.w800))]))),
        if (_counselorsError != null) ...[
          const SizedBox(height: 20),
          Container(padding: const EdgeInsets.all(14), decoration: BoxDecoration(color: const Color(0xFFFEF2F2), borderRadius: BorderRadius.circular(14), border: Border.all(color: const Color(0xFFFECACA))), child: Row(children: [const Icon(Icons.wifi_off_rounded, color: Color(0xFFDC2626), size: 18), const SizedBox(width: 10), Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [const Text('Connection issue', style: TextStyle(fontSize: 12, fontWeight: FontWeight.w800, color: Color(0xFFDC2626))), const SizedBox(height: 4), Text(_counselorsError!, style: const TextStyle(fontSize: 11, fontWeight: FontWeight.w500, color: Color(0xFF991B1B), height: 1.4))])), GestureDetector(onTap: _fetchCounselorsFromApi, child: Container(padding: const EdgeInsets.symmetric(horizontal: 12, vertical: 6), decoration: BoxDecoration(color: const Color(0xFFDC2626), borderRadius: BorderRadius.circular(8)), child: const Text('Retry', style: TextStyle(color: Colors.white, fontSize: 11, fontWeight: FontWeight.w800))))])),
        ],
      ]),
    );
  }
  ...
  Widget _buildAllocatedCard(Counselor counselor) {
    return Container(
      padding: const EdgeInsets.all(20),
      decoration: BoxDecoration(gradient: LinearGradient(colors: [kPrimaryGreen.withOpacity(0.12), Colors.white], begin: Alignment.topLeft, end: Alignment.bottomRight), borderRadius: BorderRadius.circular(24), border: Border.all(color: kPrimaryGreen.withOpacity(0.3), width: 1.5), boxShadow: [BoxShadow(color: kPrimaryGreen.withOpacity(0.1), blurRadius: 20, offset: const Offset(0, 10))]),
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        Row(children: [Container(padding: const EdgeInsets.symmetric(horizontal: 10, vertical: 5), decoration: BoxDecoration(color: kPrimaryGreen, borderRadius: BorderRadius.circular(8)), child: const Row(mainAxisSize: MainAxisSize.min, children: [Icon(Icons.verified_user_rounded, color: Colors.white, size: 14), SizedBox(width: 4), Text('Your Assigned Counselor', style: TextStyle(color: Colors.white, fontWeight: FontWeight.w800, fontSize: 12))])), const Spacer(), if (counselor.isAvailable) _buildAvailableBadge()]),
        const SizedBox(height: 18),
        Row(children: [Container(padding: const EdgeInsets.all(3), decoration: const BoxDecoration(shape: BoxShape.circle, border: Border.fromBorderSide(BorderSide(color: kPrimaryGreen, width: 2))), child: CircleAvatar(radius: 32, backgroundImage: NetworkImage(counselor.imageUrl))), const SizedBox(width: 16), Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [Text(counselor.name, style: const TextStyle(fontSize: 18, fontWeight: FontWeight.w900, color: Colors.black87)), const SizedBox(height: 4), Text(counselor.specialty, style: TextStyle(fontSize: 13, fontWeight: FontWeight.w600, color: Colors.grey.shade600)), const SizedBox(height: 4), Row(children: [const Icon(Icons.star_rounded, color: Colors.amber, size: 16), const SizedBox(width: 4), Text('${counselor.rating} • ${counselor.sessions} sessions', style: const TextStyle(fontSize: 12, fontWeight: FontWeight.w700, color: Colors.black54))])]))]),
        const SizedBox(height: 16),
        Row(children: [const Text('Contact via:', style: TextStyle(fontSize: 12, fontWeight: FontWeight.w600, color: Colors.grey)), const SizedBox(width: 8), _buildContactMethodBadge(counselor.preferredContact)]),
      ]),
    );
  }

  Widget _buildBookingFormSection() {
    return Column(key: _formSectionKey, crossAxisAlignment: CrossAxisAlignment.start, children: [
      ...
      Text('Fill in your details to request a session.', style: TextStyle(fontSize: 13, fontWeight: FontWeight.w500, color: Colors.grey.shade500)),
      const SizedBox(height: 16),
      AnimatedContainer(
        duration: const Duration(milliseconds: 500),
        padding: const EdgeInsets.all(20),
        decoration: BoxDecoration(color: _showFormSuccess ? const Color(0xFFF0FDF4) : Colors.white, borderRadius: BorderRadius.circular(24), border: _showFormSuccess ? Border.all(color: kPrimaryGreen, width: 2) : null, boxShadow: [BoxShadow(color: Colors.black.withOpacity(0.04), blurRadius: 12, offset: const Offset(0, 6))]),
        child: Form(key: _formKey, child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          if (_showFormSuccess) ...[Container(padding: const EdgeInsets.all(12), decoration: BoxDecoration(color: const Color(0xFFDCFCE7), borderRadius: BorderRadius.circular(12)), child: const Row(children: [Icon(Icons.check_circle_rounded, color: kPrimaryGreen, size: 20), SizedBox(width: 10), Expanded(child: Text('Appointment submitted successfully! Check your requests above.', style: TextStyle(fontSize: 13, fontWeight: FontWeight.w700, color: Color(0xFF166534))))])), const SizedBox(height: 16)],
          const SizedBox(height: 18), _buildFormLabel('Full Name *'), const SizedBox(height: 8), _buildFormField(controller: _nameController, hint: 'Enter your full name', icon: Icons.person_rounded, validator: (v) => (v == null || v.trim().isEmpty) ? 'Name is required' : null),
          const SizedBox(height: 18), _buildFormLabel('Contact Number *'), const SizedBox(height: 8), _buildFormField(controller: _contactController, hint: 'Enter your phone number', icon: Icons.phone_rounded, validator: (v) => (v == null || v.trim().isEmpty) ? 'Phone number is required' : null),
          const SizedBox(height: 18), _buildFormLabel('Email Address'), const SizedBox(height: 8), _buildFormField(controller: _emailController, hint: 'your@email.com (optional)', icon: Icons.email_rounded, validator: (v) { if (v != null && v.trim().isNotEmpty && !v.contains('@')) return 'Enter a valid email'; return null; }),
          const SizedBox(height: 18), _buildFormLabel('Address *'), const SizedBox(height: 8), _buildFormField(controller: _addressController, hint: 'Enter your location / address', icon: Icons.location_on_rounded, maxLines: 2, validator: (v) => (v == null || v.trim().isEmpty) ? 'Address is required' : null),
          const SizedBox(height: 18), _buildFormLabel('Service / Reason *'), const SizedBox(height: 8), _buildFormField(controller: _serviceController, hint: 'e.g., Anxiety, Depression, Stress, Relationship issues...', icon: Icons.category_rounded, validator: (v) => (v == null || v.trim().isEmpty) ? 'Service or reason is required' : null),
          const SizedBox(height: 18), _buildFormLabel('Preferred Schedule *'), const SizedBox(height: 8),
          Row(children: [
            Expanded(child: GestureDetector(onTap: () async { final picked = await showDatePicker(context: context, initialDate: DateTime.now().add(const Duration(days: 1)), firstDate: DateTime.now(), lastDate: DateTime.now().add(const Duration(days: 60)), builder: (context, child) => Theme(data: ThemeData.light().copyWith(colorScheme: const ColorScheme.light(primary: kPrimaryGreen).copyWith(secondary: kPrimaryGreen)), child: child!)); if (picked != null) setState(() => _selectedDate = picked); }, child: Container(padding: const EdgeInsets.symmetric(horizontal: 16, vertical: 16), decoration: BoxDecoration(color: Colors.white, borderRadius: BorderRadius.circular(14), border: Border.all(color: _selectedDate != null ? kPrimaryGreen : Colors.grey.shade200, width: _selectedDate != null ? 2 : 1)), child: Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [Text(_selectedDate != null ? DateFormat('MMM dd, yyyy').format(_selectedDate!) : 'Select Date', style: TextStyle(fontSize: 14, fontWeight: FontWeight.w500, color: _selectedDate != null ? Colors.black87 : Colors.grey)), Icon(Icons.calendar_month_rounded, color: _selectedDate != null ? kPrimaryGreen : Colors.grey, size: 18)])))),
            const SizedBox(width: 12),
            Expanded(child: GestureDetector(onTap: () async { final picked = await showTimePicker(context: context, initialTime: TimeOfDay.now(), builder: (context, child) => Theme(data: ThemeData.light().copyWith(colorScheme: const ColorScheme.light(primary: kPrimaryGreen).copyWith(secondary: kPrimaryGreen)), child: child!)); if (picked != null) setState(() => _selectedTime = picked); }, child: Container(padding: const EdgeInsets.symmetric(horizontal: 16, vertical: 16), decoration: BoxDecoration(color: Colors.white, borderRadius: BorderRadius.circular(14), border: Border.all(color: _selectedTime != null ? kPrimaryGreen : Colors.grey.shade200, width: _selectedTime != null ? 2 : 1)), child: Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [Text(_selectedTime != null ? _selectedTime!.format(context) : 'Select Time', style: TextStyle(fontSize: 14, fontWeight: FontWeight.w500, color: _selectedTime != null ? Colors.black87 : Colors.grey)), Icon(Icons.access_time_rounded, color: _selectedTime != null ? kPrimaryGreen : Colors.grey, size: 18)])))),
          ]),
          if (_selectedDate == null || _selectedTime == null) Padding(padding: const EdgeInsets.only(top: 8), child: Text('Please select both date and time', style: TextStyle(fontSize: 11, color: Colors.red.shade400, fontWeight: FontWeight.w600))),
          const SizedBox(height: 18), _buildFormLabel('Additional Notes'), const SizedBox(height: 8), _buildFormField(controller: _notesController, hint: 'Any additional information for the counselor...', icon: Icons.edit_note_rounded, maxLines: 3),
          const SizedBox(height: 28),
          GestureDetector(
            onTap: _isSubmitting ? null : _submitAppointment,
            child: AnimatedContainer(
              duration: const Duration(milliseconds: 200),
              width: double.infinity,
              padding: const EdgeInsets.symmetric(vertical: 18),
              decoration: BoxDecoration(gradient: _isSubmitting ? LinearGradient(colors: [Colors.grey.shade300, Colors.grey.shade300]) : LinearGradient(colors: [kPrimaryGreen, const Color.fromARGB(255, 60, 200, 65)]), borderRadius: BorderRadius.circular(18), boxShadow: _isSubmitting ? [] : [BoxShadow(color: kPrimaryGreen.withOpacity(0.4), blurRadius: 15, offset: const Offset(0, 8))]),
              child: Center(child: _isSubmitting ? const SizedBox(width: 22, height: 22, child: CircularProgressIndicator(color: Colors.white, strokeWidth: 2.5)) : const Row(mainAxisAlignment: MainAxisAlignment.center, children: [Icon(Icons.send_rounded, color: Colors.white, size: 18), SizedBox(width: 8), Text('Submit Appointment Request', style: TextStyle(color: Colors.white, fontSize: 16, fontWeight: FontWeight.w900))])),
            ),
          ),
        ])),
      ),
  ...
  Widget _buildDirectorySection() {
    return Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
      Row(children: [Container(padding: const EdgeInsets.all(8), decoration:  BoxDecoration(color: Color(0xFFEFF6FF), borderRadius: BorderRadius.circular(10)), child: const Icon(Icons.people_rounded, color: Color(0xFF2563EB), size: 18)), const SizedBox(width: 12), const Text('Counselor Directory', style: TextStyle(fontSize: 18, fontWeight: FontWeight.w800, color: Colors.black87))]),
      const SizedBox(height: 6), Text('Browse our team of professional counselors.', style: TextStyle(fontSize: 13, fontWeight: FontWeight.w500, color: Colors.grey.shade500)), const SizedBox(height: 16),
      if (_isCounselorsLoading) const Center(child: Padding(padding: EdgeInsets.symmetric(vertical: 40), child: CircularProgressIndicator())),
      if (!_isCounselorsLoading && _counselorsError != null && _allCounselors.isEmpty) _buildDirectoryEmptyError(),
      if (!_isCounselorsLoading && _allCounselors.isNotEmpty) ...[
        if (_counselorsError != null) _buildCachedBanner(),
        _buildGenderFilter(), const SizedBox(height: 16),
        if (_filteredCounselors.isNotEmpty) Padding(padding: const EdgeInsets.only(bottom: 12), child: Text('Showing ${_paginatedCounselors.length} of ${_filteredCounselors.length} counselors', style: TextStyle(fontSize: 12, fontWeight: FontWeight.w600, color: Colors.grey.shade500))),
        if (_filteredCounselors.isEmpty) _buildEmptyFilterState(),
        ..._paginatedCounselors.map((c) => _buildDirectoryCard(c)),
        if (_filteredCounselors.isNotEmpty && _totalPages > 1) ...[const SizedBox(height: 20), _buildPaginationControls()],
      ],
    ]);
  }
  ...
  Widget _buildCachedBanner() {
    return Container(margin: const EdgeInsets.only(bottom: 16), padding: const EdgeInsets.symmetric(horizontal: 14, vertical: 10), decoration: BoxDecoration(color: const Color(0xFFFFFBEB), borderRadius: BorderRadius.circular(12), border: Border.all(color: const Color(0xFFFDE68A))), child: Row(children: [const Icon(Icons.warning_amber_rounded, color: Color(0xFFD97706), size: 16), const SizedBox(width: 8), const Expanded(child: Text('Showing cached data. Tap to refresh.', style: TextStyle(fontSize: 12, fontWeight: FontWeight.w600, color: Color(0xFF92400E)))), GestureDetector(onTap: _fetchCounselorsFromApi, child: const Icon(Icons.refresh_rounded, color: Color(0xFFD97706), size: 18))]));
  }
  ...
  Widget _buildDirectoryCard(Counselor counselor) {
    return Container(
      margin: const EdgeInsets.only(bottom: 12),
      padding: const EdgeInsets.all(16),
      decoration: BoxDecoration(color: Colors.white, borderRadius: BorderRadius.circular(20), border: Border.all(color: counselor.isAvailable ? Colors.grey.shade100 : Colors.grey.shade200), boxShadow: [BoxShadow(color: Colors.black.withOpacity(0.03), blurRadius: 8, offset: const Offset(0, 3))]),
      child: Row(children: [
        CircleAvatar(radius: 26, backgroundImage: NetworkImage(counselor.imageUrl)),
        const SizedBox(width: 14),
        Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          Row(children: [Expanded(child: Text(counselor.name, style: const TextStyle(fontSize: 15, fontWeight: FontWeight.w800, color: Colors.black87))), const SizedBox(width: 6), Icon(counselor.gender == 'Male' ? Icons.male_rounded : Icons.female_rounded, color: counselor.gender == 'Male' ? Colors.blue.shade300 : Colors.pink.shade300, size: 15)]),
          const SizedBox(height: 4), Text(counselor.specialty, style: TextStyle(fontSize: 13, fontWeight: FontWeight.w600, color: Colors.grey.shade500)), const SizedBox(height: 8),
          Row(children: [const Icon(Icons.star_rounded, color: Colors.amber, size: 14), const SizedBox(width: 4), Text('${counselor.rating}', style: const TextStyle(fontSize: 12, fontWeight: FontWeight.w700, color: Colors.black54)), const SizedBox(width: 12), Text('${counselor.sessions} sessions', style: TextStyle(fontSize: 12, fontWeight: FontWeight.w600, color: Colors.grey.shade400)), if (counselor.isAvailable) ...[const SizedBox(width: 12), _buildAvailableBadge()]]),
        ])),
        if (!counselor.isAvailable) Container(padding: const EdgeInsets.symmetric(horizontal: 10, vertical: 6), decoration: BoxDecoration(color: const Color(0xFFF3F4F6), borderRadius: BorderRadius.circular(10)), child: const Text('Unavailable', style: TextStyle(fontSize: 11, fontWeight: FontWeight.w700, color: Color(0xFF9CA3AF)))),
      ]),
    );
  }
  ...
  List<Widget> _buildPageNumbers() {
    ...
    final pages = <int?>[];
    if (total <= 5) { for (int i = 1; i <= total; i++) pages.add(i); } else { pages.add(1); if (_currentPage > 3) pages.add(null); for (int i = (_currentPage - 1).clamp(2, total - 1); i <= (_currentPage + 1).clamp(2, total - 1); i++) { if (!pages.contains(i)) pages.add(i); } if (_currentPage < total - 2) pages.add(null); if (!pages.contains(total)) pages.add(total); }
    return pages.map((page) {
      ...
      return Padding(padding: const EdgeInsets.symmetric(horizontal: 3), child: GestureDetector(onTap: () => setState(() => _currentPage = page), child: AnimatedContainer(duration: const Duration(milliseconds: 200), width: 36, height: 36, decoration: BoxDecoration(color: isCurrent ? kPrimaryGreen : Colors.transparent, borderRadius: BorderRadius.circular(10), border: isCurrent ? null : Border.all(color: Colors.grey.shade200)), child: Center(child: Text('$page', style: TextStyle(fontSize: 13, fontWeight: FontWeight.w800, color: isCurrent ? Colors.white : Colors.grey.shade600))))));
    }).toList();
  }
  ...
  Widget build(BuildContext context) {
    if (_isRoleLoading) {
      return const Scaffold(backgroundColor: Color(0xFFF5F7F5), body: Center(child: CircularProgressIndicator()));
    }

    // 🔒 DEBUG: Show current role in a banner so you can see what's stored
    print('🔑 BUILD: _userRole = "$_userRole", _canAccessCounselorFeatures = $_canAccessCounselorFeatures');
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        leading: Container(
          ...
          child: IconButton(icon: const Icon(Icons.arrow_back_ios_new, color: Colors.black87, size: 20), onPressed: () => Navigator.pop(context)),
        ),
        title: const Text('Your Counselor', style: TextStyle(color: Colors.black87, fontWeight: FontWeight.w900, fontSize: 20)),
        centerTitle: true,
        actions: [
          // ✅ Everyone sees "My Appointments"
          GestureDetector(
            onTap: _showMyAppointmentsSheet,
            child: Container(
              margin: const EdgeInsets.only(right: 8),
              padding: const EdgeInsets.symmetric(horizontal: 12, vertical: 8),
              decoration: BoxDecoration(color: kPrimaryGreen.withOpacity(0.15), borderRadius: BorderRadius.circular(20), border: Border.all(color: kPrimaryGreen.withOpacity(0.3))),
              child: Row(mainAxisSize: MainAxisSize.min, children: [
                const Icon(Icons.history_rounded, color: kPrimaryGreen, size: 18),
                if (_userAppointments.isNotEmpty) ...[
                  const SizedBox(width: 6),
                  Container(padding: const EdgeInsets.all(4), decoration: const BoxDecoration(color: kPrimaryGreen, shape: BoxShape.circle), child: Text(_userAppointments.length.toString(), style: const TextStyle(color: Colors.white, fontSize: 10, fontWeight: FontWeight.w800))),
                ],
              ]),
            ),
          ),

          // 🔒 COUNSELOR ONLY: Calendar icon
          if (_canAccessCounselorFeatures)
            GestureDetector(
              onTap: _showCounselorAppointmentsPanel,
              child: Container(
                margin: const EdgeInsets.only(right: 8),
                decoration: BoxDecoration(color: const Color(0xFF7C3AED), shape: BoxShape.circle, boxShadow: [BoxShadow(color: const Color(0xFF7C3AED).withOpacity(0.3), blurRadius: 10)]),
                child: Stack(children: [
                  const Padding(padding: EdgeInsets.all(12), child: Icon(Icons.calendar_today_rounded, color: Colors.white, size: 20)),
                  if (_pendingCount > 0)
                    Positioned(right: 0, top: 0, child: Container(
                      ...
                      child: Text(_pendingCount.toString(), textAlign: TextAlign.center, style: const TextStyle(color: Colors.white, fontSize: 10, fontWeight: FontWeight.w800)),
                    )),
                ]),
              ),
            ),

          // 🔒 COUNSELOR ONLY: Shield/session log icon
          if (_canAccessCounselorFeatures)
            Container(
              margin: const EdgeInsets.only(right: 16),
              decoration: BoxDecoration(color: const Color(0xFF7C3AED).withOpacity(0.15), shape: BoxShape.circle, border: Border.all(color: const Color(0xFF7C3AED).withOpacity(0.3))),
              child: IconButton(icon: const Icon(Icons.shield_rounded, color: Color(0xFF7C3AED), size: 20), onPressed: _showCounselorSessionLogSheet, tooltip: 'Session Log'),
            ),
        ],
      ),
      body: SingleChildScrollView(
        ...
        child: Column(
          crossAxisAlignment: CrossAxisAlignment.start,
          children: [
            // 🔒 DEBUG: Temporary role banner — REMOVE after confirming it works
            Container(
              width: double.infinity,
              padding: const EdgeInsets.all(12),
              margin: const EdgeInsets.only(bottom: 16),
              decoration: BoxDecoration(
                color: _canAccessCounselorFeatures ? const Color(0xFFF5F3FF) : const Color(0xFFFFFBEB),
                borderRadius: BorderRadius.circular(12),
                border: Border.all(color: _canAccessCounselorFeatures ? const Color(0xFF7C3AED) : const Color(0xFFFDE68A)),
              ),
              child: Row(children: [
                Icon(_canAccessCounselorFeatures ? Icons.verified_rounded : Icons.info_outline_rounded, color: _canAccessCounselorFeatures ? const Color(0xFF7C3AED) : const Color(0xFFD97706), size: 18),
                const SizedBox(width: 10),
                Expanded(child: Text(
                  'Role: "$_userRole" • Counselor features: ${_canAccessCounselorFeatures ? "VISIBLE ✅" : "HIDDEN ❌"}',
                  style: TextStyle(fontSize: 12, fontWeight: FontWeight.w700, color: _canAccessCounselorFeatures ? const Color(0xFF7C3AED) : const Color(0xFF92400E)),
                )),
              ]),
            ),
